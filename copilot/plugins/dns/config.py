...
class ConfigWriter(Config):
    ...
    def __init__(self):
        ...
        The header [A] defines the record type that DNSChef
        should apply its DOMAIN=RECORD pairs to when it reads
        the external definitions file.
        https://thesprawl.org/projects/dnschef/#external-definitions-file
        """
    ...
    def get_dns(self, sub):
        """Get the DNSChef target stanza to apply the action to.

        DNSChef uses DOMAIN=RECORD pairs to filter domains. This function tries
        to identify the filtering DOMAIN that should be used and transform the
        supplied string by adding the appropriate wildcards to match the users
        intention. In depth information on filtering methods can be found in
        the DNSChef documentation.
        https://thesprawl.org/projects/dnschef/#filtering-domains
        """
        # Currently uses a hard-coded list of TLD's. This is not ideal.
